import { getDaysOfWeek } from './date';

describe('getDaysOfWeek', () => {
    beforeAll(() => {
        jest.useFakeTimers({
            now: new Date('2023-09-01T00:00:00.000Z'),
        });
    });

    it('returns an array of days of the week', () => {
        const days = getDaysOfWeek(new Date());

        expect(days).toStrictEqual([
            {
                label: 'Mon 28/08/2023',
                timestamp: new Date('2023-08-28T00:00:00.000Z'),
            },
            {
                label: 'Tue 29/08/2023',
                timestamp: new Date('2023-08-29T00:00:00.000Z'),
            },
            {
                label: 'Wed 30/08/2023',
                timestamp: new Date('2023-08-30T00:00:00.000Z'),
            },
            {
                label: 'Thu 31/08/2023',
                timestamp: new Date('2023-08-31T00:00:00.000Z'),
            },
            {
                label: 'Fri 01/09/2023',
                timestamp: new Date('2023-09-01T00:00:00.000Z'),
            },
            {
                label: 'Sat 02/09/2023',
                timestamp: new Date('2023-09-02T00:00:00.000Z'),
            },
            {
                label: 'Sun 03/09/2023',
                timestamp: new Date('2023-09-03T00:00:00.000Z'),
            },
        ]);
    });
});
